...
import {
  createOrder,
  approveNFT1155Influencer,
  isApprovedNFT1155Influencer,
...
export default function SellPage(props) {
  ...
  const balanceNFT = useSelector((state) => state?.data.userInfluencerNFTs);
  ...
  const handleCreate = async () => {
    ...
    if (props.location.nft?.metaverse?.length) {
      props.location.nft.metaverse === true ||
      props.location.nft.metaverse === "true"
        ? setMetaverse(true)
        : setMetaverse(false);
    } else {
  ...
  const handleApprove = async () => {
    setIsLoading(true);
    let result = await approveNFT1155Influencer();
    if (result) {
      checkApproval();
    }
    setIsLoading(false);
  ...
  const checkApproval = async () => {
    let result = await isApprovedNFT1155Influencer();
    if (result) {
  ...
  return (
    <Paper className="form_content">
      <Typography variant="h5">Trade Influencer Experiences</Typography>
      <Grid container spacing={3}>
        ...
        >
          <Typography variant="h5">{NFTinfo.name}</Typography>
          <Box
            style={{
              width: "300px",
              // height: "300px",
          ...
          <Grid container>
            <Grid xs={12} lg={6} className="column">
              <form className={classes.root} noValidate autoComplete="off">
                ...
                <div>
                  <a
                    href={NFTinfo.external_url}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <h2>{NFTinfo.external_url}</h2>
                  </a>
                </div>
                <TextareaAutosize
