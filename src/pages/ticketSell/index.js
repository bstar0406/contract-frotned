...
import {
  createOrder,
  approveNFT1155Tickets,
  isApprovedNFT1155Tickets,
...
export default function SellPage(props) {
  ...
  const balanceNFT = useSelector((state) => state?.data.userTicketsNFTs);
  ...
  const handleApprove = async () => {
    setIsLoading(true);
    let result = await approveNFT1155Tickets();
    if (result) {
      checkApproval();
    }
    setIsLoading(false);
  ...
  const checkApproval = async () => {
    let result = await isApprovedNFT1155Tickets();
    if (result) {
  ...
  useEffect(async () => {
    if (props.location.nft) {
      setNFTinfo({ ...props.location.nft, sendType: "TICKET" });
    } else {
      handleSelectChange(id);
    }
    checkApproval();
  ...
  return (
    <Paper className="form_content">
      <Typography variant="h5">
        Trade {NFTinfo.metaverse ? "Metaverse" : "Ticket"} Assets
      </Typography>
          ...
          <Grid container>
            <Grid xs={12} lg={6} className="column">
              <form className={classes.root} noValidate autoComplete="off">
                ...
                />
                {NFTinfo.links && NFTinfo.links.length !== 0 && (
                  <div>
                    <h3> links </h3>
                    {NFTinfo.links.map((link, index) => {
                      return (
                        <a
                          key={index}
